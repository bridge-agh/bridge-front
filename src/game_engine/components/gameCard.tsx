...
import { BufferAttribute, BufferGeometry, DoubleSide, MeshBasicMaterial, MeshStandardMaterial, RepeatWrapping, TextureLoader, sRGBEncoding } from "three";

function RoundEdgedBoxFlat({ w, h, t, r, s }: { w: number, h: number, t: number, r: number, s: number }) { // width, height, thick, radius corner, smoothness
...
export function GameCard({
  ...
  const cardMap = useLoader(TextureLoader, "png/cards/dark/7S.png");
  ...
  const realScale = z + scaleFactor;

  return (
    <>
      <mesh ref={ref} position={[x, y, realScale]} scale={scale} rotation={[0, 0, rotation]} geometry={geometry} material={material} onPointerOver={
        () => {
          setScaleFactor(0.1);
          console.log("over");
        }
